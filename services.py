...
def administrar_chatbot(text,number, messageId, name):
    ...
    if "hola" in text:
        body = "¡Hola! 👋 Te comunicas con Agustin de ViaSano Salud \n \nEl motivo de mi mensaje es ayudar aquellas personas que no dieron de alta o están desconformes con su cobertura medica. \n \n Desde Viasano salud queremos  brindarte información valiosa para que compares con tu actual OBRA SOCIAL "
        footer = "Equipo Via Sano Bouzas ©"
        options = ["✅ Quiero comparar", "🚫 Estoy ocupado", "✅ Particular",]

        replyButtonData = buttonReply_Message(number, options, body, footer, "sed1",messageId)
        replyReaction = replyReaction_Message(number, messageId, "🫡")
        list.append(replyReaction)
        list.append(replyButtonData)
    elif "✅ Quiero comparar" in text:
        textMessage = text_Message(number,"Contamos con Planes Exclusivos.")

        body = "Tenemos una excelente opción de salud para vos y tu familia !!!👍😊 /nCONOCE NUESTROS BENEFICIOS EXCLUSIVOS 😁👇"
        footer = "Equipo Via Sano Bouzas ©"
        options = ["Recibo de Sueldo", "Monotributista", "Particular"]
